...
using cm = frontlook_csharp_library.FL_General.FL_ConsoleManager;
...
namespace frontlook_csharp_library.FL_General
{
	public static class FL_AsciiSerial
	{
		...
		public static string FL_SetNewCode(this string code, bool DebugMode = false)
		{
			...
			{
				var LastCode = code;
				cm.FL_ConsoleWriteDebug($"\nLast Code: {LastCode}");
				var Z_Asc = FL_Z_Asc(LastCode, DebugMode);
				cm.FL_ConsoleWriteDebug($"New Ascii val : {Z_Asc}");
				cm.FL_ConsoleWriteDebug($"New Ascii val (Long): {(long)Z_Asc}");
				var Z_Chr = FL_Z_Chr((long)Z_Asc + 1L, DebugMode);
				cm.FL_ConsoleWriteDebug($"New Code In Char : {Z_Chr}\n");
				return Z_Chr;
			}
			else
			{
		...
		public static string FL_GetAsciiCodeFromString(this long code, bool DebugMode = false)
		{
			if (DebugMode)
			{
				var LastCode = code;
				var Z_Chr = FL_Z_Chr(code, DebugMode);
				cm.FL_ConsoleWriteDebug($"New Code In Char : {Z_Chr}\n");
				return Z_Chr;
			}
			else
			{
				return FL_Z_Chr(code);
			}
		}
